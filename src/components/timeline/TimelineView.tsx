...
function TimelineView({ events }) {
    ...
    return (<div className="flow-root">
      <ul role="list" className="-mb-8">
        {events.map((event, eventIdx) => (<li key={event.id}>
            <div className="relative pb-8">
              {eventIdx !== events.length - 1 ? (<span className="absolute top-4 left-4 -ml-px h-full w-0.5 bg-gray-200" aria-hidden="true"/>) : null}
              <div className="relative flex space-x-3">
                ...
                <div className="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                  ...
                  <div className="whitespace-nowrap text-right text-sm text-gray-500">
                    {(0, date_fns_1.format)(new Date(event.date), 'MMM d, yyyy')}
                  </div>
                </div>
              </div>
            </div>
